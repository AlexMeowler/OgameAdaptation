...
import org.retal.offgame.old.core.*;
import org.retal.offgame.old.core.buildings.Building;
import org.retal.offgame.old.core.units.Unit;
import org.retal.offgame.old.core.units.UnitBuildQueueElement;

public class SpaceYardDefensePanel extends InfoPanel 
{
	public SpaceYardDefensePanel(String name, Player player) throws IOException 
	{
		super(name, player);
		setOpaque(false);
		...
		constraints.weightx = 0.0f;
		y_offset = 1;
		requirements_panels = new RequirementsPanel[Unit.DEFENSE_AMOUNT];
		for(int i = 0; i < Unit.DEFENSE_AMOUNT; i++)
		{
			addRow(Unit.ROCKET_LAUNCHER + i, i + y_offset);
		}
		y_offset_after = y_offset + Unit.DEFENSE_AMOUNT;
		constraints.gridx = 1;
	...
	private void addRow(int code, int row) throws IOException
	{
		...
		requirements_panels[row - y_offset] = new RequirementsPanel(buildings_required, technologies_required, current_planet.getBuildings(), player.getTechs());
		add(requirements_panels[row - y_offset], constraints);
		//
		constraints.fill = GridBagConstraints.BOTH;
	...
	public void updatePanelUI()
	{
		...
		for(int i = 0; i < list.length; i++)
		{
			if(list[i] instanceof TextLabel)
			{
				...
				{
					String header = current_planet.getUnits()[coords[1] - y_offset + Unit.ROCKET_LAUNCHER].generateHeader();
					double[] resources = {current_planet.getCurrentMetal(), current_planet.getCurrentCrystal(), current_planet.getCurrentDeiterium(), current_planet.getCurrentEnergy()};
					String description = current_planet.getUnits()[coords[1] - y_offset + Unit.ROCKET_LAUNCHER].generateDescription(resources);
					if(coords[0] == 1)
					{
						((TextLabel)list[i]).setText(header + "<br>" + description + createTimeString(coords[1] - y_offset + Unit.ROCKET_LAUNCHER));
					}
				}
			...
			else
			{
				if(list[i] instanceof BuildAndInputField)
				{
					...
					boolean requirements_flag = current_planet.requirementsMet(EntityCategory.DEFENSE, coords[1] + Unit.ROCKET_LAUNCHER - y_offset);
	...
	private class BuildAndInputField extends JPanel implements ActionListener, KeyListener, FocusListener
	{
		...
		public void actionPerformed(ActionEvent e)
		{
			...
			int code = Unit.ROCKET_LAUNCHER + Integer.parseInt(s[1]) - y_offset;
			try
			{
